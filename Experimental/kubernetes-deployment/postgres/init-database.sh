...
# IMPORT ESDL-DRIVE AND KEYCLOAK BACKUPS IF THEY EXIST
if [ -f /keycloak.sql ]; then
  echo -e "KEYCLOAK BACKUP DETECTED, IMPORTING DATABASE..."
  psql -h localhost -p 5432 -d keycloak -U keycloak -f /keycloak.sql
fi

sleep 3

if [ -f /drive.sql ]; then
  echo -e "ESDL-DRIVE BACKUP DETECTED, IMPORTING DATABASE..."
  psql -h localhost -p 5432 -d esdlrepo -U drive -f /drive.sql
fi